...
function Publish-ToCheckRun {
    ...
    if ($ctx.EventName -eq 'pull_request') {
        Write-ActionInfo "DEBUG: GitHub Event context"
		Write-ActionInfo (Get-Content $env:GITHUB_EVENT_PATH)
    ...
    $result = Invoke-WebRequest -Headers $hdr $url -Method Post -Body ($bdy | ConvertTo-Json)
	Write-ActionInfo $result.RawContent
}
