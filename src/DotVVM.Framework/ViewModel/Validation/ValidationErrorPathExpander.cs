...
namespace DotVVM.Framework.ViewModel.Validation
{
    ...
    internal class ValidationErrorPathExpander : IValidationErrorPathExpander
    {
        ...
        public void Process(ModelState modelState, object viewModel)
        {
            ...
            // Check that model state has some paths that need to be expanded
            if (modelState.Errors.All(error => error.PropertyPath.StartsWith("/")))
            {
                // All paths are already in the correct form
                return;
            }
            ...
            Process(viewModel, string.Empty, modelStateDecoratorContext);
        ...
        private void Process(object? viewModel, string pathPrefix, ValidationErrorPathExpanderContext context)
        {
            ...
            if (ReflectionUtils.IsEnumerable(viewModelType))
            {
                ...
                foreach (var item in (IEnumerable)viewModel)
                    Process(item, $"{pathPrefix}/{index++}", context);
            ...
            else
            {
                ...
                foreach (var property in map.Properties.Where(p => p.TransferToServer))
                {
                    ...
                    if (ReflectionUtils.IsComplexType(property.Type))
                        Process(value, $"{pathPrefix}/{property.Name}", context);
                }
            }
