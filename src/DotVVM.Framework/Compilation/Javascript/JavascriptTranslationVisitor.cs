...
namespace DotVVM.Framework.Compilation.Javascript
{
    public class JavascriptTranslationVisitor
    {
        ...
        private Expression ReplaceVariables(Expression node, IReadOnlyList<ParameterExpression> variables, CodeSymbolicParamerer[] args)
        {
        ...
        {
            var args = expression.Parameters.Select(p => new CodeSymbolicParamerer($"lambda_param_" + p.Name)).ToArray();
        ...
        (JsNode node, object[] variables, string[] variableNames) TranslateLambdaBody(Expression expression)
        {
            ...
            {
                var args = block.Variables.Select(_ => new CodeSymbolicParamerer()).ToArray();
