...
namespace DotVVM.Framework.Compilation.Binding
{
    public class ExpressionBuildingVisitor : BindingParserNodeVisitor<Expression>
    {
        ...
        private Type FindCommonType(Type type1, Type type2)
        {
            ...
            var commonInterfaces = interfaces1.Intersect(interfaces2).ToArray();

            if (commonInterfaces.Length > 0)
            {
                // Return the most specific common interface
                return commonInterfaces.OrderBy(i => i.GetInterfaces().Length).FirstOrDefault() ?? typeof(object);
            }
