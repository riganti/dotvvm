...
namespace DotVVM.Framework.Compilation.Parser.Binding.Parser
{
    public class BindingParser : ParserBase<BindingToken, BindingTokenType>
    {
        ...
        private BindingParserNode ReadAtomicExpression()
        {
            ...
            var token = Peek();
            if (token != null && token.Type == BindingTokenType.NewKeyword)
            {
                // constructor call expression
                return ReadConstructorCallExpression();
            }
            else if (token != null && token.Type == BindingTokenType.OpenParenthesis)
            {
        ...
        private BindingParserNode ReadConstructorCallExpression()
        {
            ...
            if (Peek()?.Type != BindingTokenType.NewKeyword)
            {
